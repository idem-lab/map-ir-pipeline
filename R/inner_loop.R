...
inner_loop <- function(data,
  ...
  in_sample_predictions_rf <- predict(
    # the new environmental covariates
    zero_level_in_sample_rf,
    new_data
  )

  in_sample_predictions_xgb <- predict(
    # the new environmental covariates
    zero_level_in_sample_xgb,
    new_data
  )

  in_sample_covariates <- tibble(
    .pred_randomForest = in_sample_predictions_rf$.pred,
